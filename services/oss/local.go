...
func (*local) Upload(file multipart.File, fileHeader *multipart.FileHeader, uploadDir ...string) (*UploadRet, error) {
	...
	// 检查并创建上传目录
	isPath, mkdirErr := bvutils.FileIsExist(savePathUri)
	if mkdirErr != nil {
		return nil, mkdirErr
	}
	if !isPath {
	...
	if err := bvutils.SaveFile(fileHeader, filePath); err != nil {
		return nil, err
	}

	return &UploadRet{
		Hash:     bvutils.Md5Encode(filePath),
		Filename: filename,
		Url:      bvutils.AssembleServerPath(filePath),
	}, nil
}
